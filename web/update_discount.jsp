                ...
                <div class="card-body">
                    <div class="form-grid">
                        <div class="form-group">
                            ...
                            <input type="text" id="code" name="code" class="form-control" value="${discount.discountCode}"  required>
                        </div>
                        <div class="form-group">
                            <label for="type">Loại giảm</label>
                            <select id="type" name="type" class="form-control"  >
                                <option value="Percent" ${discount.discountType == 'Percent' ? 'selected' : ''}>Percent</option>
                                <option value="Fixed" ${discount.discountType == 'Fixed' ? 'selected' : ''}>Fixed</option>
                            </select>
                        </div>
                        <div class="form-group">
                            <label for="value">Giá trị</label>
                            <input type="number" step="0.01" id="value" name="value" class="form-control" value="${discount.discountValue}"  required>
                        </div>
                        <div class="form-group">
                            <label for="minAmount">Đơn hàng tối thiểu (₫)</label>
                            <input type="number" step="100" id="minAmount" name="minAmount" class="form-control" value="${discount.minOrderAmount}"  >
                        </div>
                        <div class="form-group">
                            <label for="maxValue">Giảm tối đa (₫)</label>
                            <input type="number" step="100" id="maxValue" name="maxValue" class="form-control"
                                   value="${discount.maxValue != null ? discount.maxValue : ''}"  >
                        </div>
                        <div class="form-group">
                            <label for="maxUsage">Số lần dùng tối đa</label>
                            <input type="number" id="maxUsage" name="maxUsage" class="form-control"
                                   value="${discount.maxUsage != null ? discount.maxUsage : ''}"  >
                        </div>
                        <div class="form-group">
                            <label for="validFrom">Hiệu lực từ</label>
                            <input type="date" id="validFrom" name="validFrom" class="form-control"
                                   value="<fmt:formatDate value='${discount.validFrom}' pattern='yyyy-MM-dd'/>"  required>
                        </div>
                        <div class="form-group">
                            <label for="validTo">Tới ngày</label>
                            <input type="date" id="validTo" name="validTo" class="form-control"
                                   value="<fmt:formatDate value='${discount.validTo}' pattern='yyyy-MM-dd'/>"  required>
                        </div>
                        <div class="form-group">
                            <label for="status">Trạng thái</label>
                            <select id="status" name="status" class="form-control"   >
                                <option value="1" ${discount.active ? 'selected' : ''}>Active</option>
                                <option value="0" ${!discount.active ? 'selected' : ''}>Deactive</option>
                            </select>
                        </div>
                        <div class="form-full-width">
                            <label for="description">Mô tả</label>
                            <textarea id="description" name="description" class="form-control" rows="4"  >${discount.description}</textarea>
                        </div>
                    </div>
                </div>
