...
//            request.getRequestDispatcher("profile_account_page.jsp").forward(request, response);
            ...
            byte[] imageData = null;
            Part avatarPart = request.getPart("images");
            ...
            if (imageData == null) {
                try {
                    String defaultImagePath = getServletContext().getRealPath("/images/defaultprofile.png");
                    File defaultImageFile = new File(defaultImagePath);

                    if (defaultImageFile.exists()) {
                        imageData = Files.readAllBytes(defaultImageFile.toPath());
                    }
                } catch (IOException e) {
                    e.printStackTrace();
                }
            ...
            if (imageData != null) {
                boolean imageSuccess = accDao.uploadAvatar(acc.getAccId(), imageData);

                if (imageSuccess) {
                    acc.setAccImage(imageData);
                    session.setAttribute("userAccount", acc);
                    request.setAttribute("updateSucess", "Cập nhật Avatar thành công");
                } else {
                    request.setAttribute("errMess", "Lỗi lưu ảnh vào cơ sở dữ liệu");
                }
            } else {
                request.setAttribute("errMess", "Không có ảnh nào được tải lên");
            }
