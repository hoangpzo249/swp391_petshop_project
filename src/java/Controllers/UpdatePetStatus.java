...
public class UpdatePetStatus extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        if (_daopet.updatePetStatusById(petId, status)) {
            switch (status) {
                case 1:
                    session.setAttribute("successMess", "Thú cưng " + petId + " đã hiện thành công.");
                    break;
                case 0:
            ...
            session.setAttribute("errorMess", "Thực hiện hành động không thành công.");
        }
        if (referer != null) {
